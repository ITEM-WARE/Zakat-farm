...
import { Search, ArrowUpDown, Trash2, Edit, Eye, Download, User, AlertCircle, AlertTriangle, Filter, X, ListFilter } from 'lucide-react';
...
export function ProfileList() {
  ...
  const profiles = useMemo(() => {
    ...
    Object.entries(questionFilters).forEach(([qId, value]) => {
      if (value) {
        filtered = filtered.filter(p => {
          const answer = p.answers?.[qId];
          const q = settings?.questions?.find(q => q.id === qId);

          if (q?.type === 'range') {
            const numValue = Number(answer);
            if (isNaN(numValue)) return false;
            const option = q.options?.find(o => o.label === value);
            if (!option) return false;
            const min = (option.min === undefined || option.min === null || isNaN(option.min)) ? -Infinity : option.min;
            const max = (option.max === undefined || option.max === null || isNaN(option.max)) ? Infinity : option.max;
    ...
    return filtered;
  }, [allProfiles, searchTerm, sortBy, selectedDistrict, questionFilters, limit]);
  ...
  const handleExportCSV = async () => {
    ...
    const headers = [
      ...
      ...dynamicHeaders, ...conditionalHeaders, ...followUpHeaders, 'Score', 'Created At', 'Updated At'
    ];
  ...
  const activeFilterCount = (selectedDistrict ? 1 : 0) + Object.values(questionFilters).filter(Boolean).length + (limit > 0 ? 1 : 0);
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                {settings?.questions?.map(q => (
                  <div key={q.id} className="space-y-1">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider line-clamp-1" title={q.text}>
                      {q.text}
                    </label>
                    <select
                      value={questionFilters[q.id] || ''}
                      onChange={(e) => setQuestionFilters(prev => ({ ...prev, [q.id]: e.target.value }))}
                      className="w-full py-2 pl-3 pr-8 border border-slate-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none bg-white transition-all text-sm"
                    >
